...
class AcrTransAct_CNN(pl.LightningModule):
    """
    ...
    def __init__(self, params, channels_first=True, debug_mode=False, verbose=False):
        ...
        self.kernel_size_sf = params["kernel_size_sf"] if self.use_sf else 0
        ...
        self.num_layers = params["num_main_layers"]  # determines the number of CONV layers
        ...
        if self.use_aa:
            self.conv_aa = nn.Conv1d(
                in_channels=self.hidden_size_aa
                if self.channels_first
                else self.seq_len_aa,
                out_channels=self.out_channels_aa,
                kernel_size=self.kernel_size_aa,
            )

            if self.num_layers == 2:
                self.conv_aa2 = nn.Conv1d(
                    in_channels=self.out_channels_aa,
                    out_channels=self.out_channels_aa,
                    kernel_size=self.kernel_size_aa,
                )
                torch.nn.init.xavier_uniform_(self.conv_aa2.weight)

            torch.nn.init.xavier_uniform_(self.conv_aa.weight)
            self.bn1_aa = nn.BatchNorm1d(self.out_channels_aa)

        if self.use_sf:
            self.conv_sf = nn.Conv1d(
                in_channels=self.hidden_size_sf
                if self.channels_first
                else self.seq_len_sf,
                out_channels=self.out_channels_sf,
                kernel_size=self.kernel_size_sf,
            )

            if self.num_layers == 2:
                self.conv_sf2 = nn.Conv1d(
                    in_channels=self.out_channels_sf,
                    out_channels=self.out_channels_sf,
                    kernel_size=self.kernel_size_sf,
                )
                torch.nn.init.xavier_uniform_(self.conv_sf2.weight)

            torch.nn.init.xavier_uniform_(self.conv_sf.weight)
            self.bn1_sf = nn.BatchNorm1d(self.out_channels_sf)
        ##########################################################
        if self.channels_first:  # channels first mode (batch, channels, seq_len)
            in_features_aa = self.out_channels_aa * (
                self.seq_len_aa
                - (self.num_layers * self.kernel_size_aa)
                + (self.num_layers * 1)
            )

            in_features_sf = self.out_channels_sf * (
                self.seq_len_sf
                - (self.num_layers * self.kernel_size_sf)
                + (self.num_layers * 1)
            )
    ...
    def forward(self, x):
        ...
        if self.use_aa:
            if self.debug_mode:
                print("1 x_aa shape before conv: ", x_aa.shape)
            x_aa = self.conv_aa(x_aa)
            ...
            if self.num_layers == 2:
                x_aa = self.conv_aa2(x_aa)
                x_aa = self.bn1_aa(x_aa)
                x_aa = F.relu(x_aa)
                if self.dout != 0:
                    x_aa = self.dropout(x_aa)
                if self.debug_mode:
                    print("2 x_aa shape after conv2: ", x_aa.shape)
        ...
        if self.use_sf:
            if self.debug_mode:
                print("1 x_sf shape before conv: ", x_sf.shape)

            x_sf = self.conv_sf(x_sf)

            if self.debug_mode:
                print("2 x_sf shape after conv: ", x_sf.shape)
            ...
            if self.num_layers == 2:
                x_sf = self.conv_sf2(x_sf)
                x_sf = self.bn1_sf(x_sf)
                x_sf = F.relu(x_sf)
                if self.dout != 0:
                    x_sf = self.dropout(x_sf)
                if self.debug_mode:
                    print("2 x_sf shape after conv2: ", x_sf.shape)

            x_sf = x_sf.view(x_sf.shape[0], -1)
            if self.debug_mode:
                print("3 x_sf shape after view: ", x_sf.shape)
        ##########################################################
        if self.use_sf and self.use_aa:
            # concat the two conv layers outputs
            if self.debug_mode:
                print("4 x before concat ss and aa shape: ", x_aa.shape, x_sf.shape)
        ...
        if self.mode >= 1:
            if self.debug_mode:
                ...
                print("6 weight of fc1: ", self.fc1.weight.shape)
            x = self.fc1(x)
            if self.debug_mode:
